...
namespace
{
    ...
    __host__ __device__ int SignConfig(float isoLevel, float *sdf)
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            {
                //printf("sdf %d: %f\n",i,sdf[i]);
                //printf("*********LESS THAN INDEXES: %d, %d, %d\n",ix,iy,iz);
                edgeIndex |= 1 << i;
            }
        }
        ...
        float3 *intersect)
{
    ...
    {
        //printf("signConfig: %d",signConfig);
        return;
    }
    //printf("Not returned\n");
        ...
        int cubesPerThread)
{
    ...
    // each thread calculates cubesPerThread number of cubes
    int threadIndex = (blockIdx.x * blockDim.x + threadIdx.x) * cubesPerThread;
    /// this is for creating 3d indexes from 1d grid 1d blocks
    int ix = threadIndex / (NumZ * NumY);
    int iy = (threadIndex - ix * NumZ * NumY) / NumZ;
    int iz = threadIndex - NumZ * (iy + NumY * ix);

    for (int i = 0; i < cubesPerThread; i++){
        if (iz == NumZ){
            iz = 0;
            iy ++;
        }
        ...
        if (ix < NumX && iy < NumY && iz < NumZ && ix >= 0 && iy >= 0 && iz >= 0)
        {
            float3 intersect[12];
            float3 min = make_float3(x, y, z);
        ...
        int cubesPerThread)
{
    ...
    int offsetPerFrame = NumX * NumY * NumZ * 16;
    //int offsetPerFrame = NumX * NumY * NumZ * 16 * sizeof(float3);
    ...
    for (int frame = 0; frame < frameNum; frame++){
        ...
        int offsetPerCurrentFrame = frame * offsetPerFrame;
        ...
        for (int i = 0; i < cubesPerThread; i++){
            ...
            if (ix < NumX && iy < NumY && iz < NumZ && ix >= 0 && iy >= 0 && iz >= 0)
            {
                ...
                // now create and store the triangle data
                //int offset = (NumZ * NumY * ix + NumZ * iy + iz) * 16;
                int offset = (NumZ * NumY * ix + NumZ * iy + iz) * 16 + offsetPerCurrentFrame;
                Triangulate(twist, meshVertices + offset, meshNormals + offset, signConfig, intersect);
            }
            iz++;
        }
...
int main(int argc, char *argv[])
{
    ...
    printf("resolution (n) = %d, number of frames (f) = %d\nblocks (b) = %d, threads (t) = %d\n", cubesRes, frameNum, numBlocks, numThreads);
    ...
    int cubesPerThread = totalCubes / totalThreads + (totalCubes % totalThreads == 0 ? 0 : 1);
    if (cubesPerThread < 1) cubesPerThread = 1;
    printf("CUBES PER THREAD: %d\n",cubesPerThread);
    ///
    double kernelTime = 0;
    double memcpyTime = 0;
    double extraTime = 0;
    ...
    if (part == -1 || part == 1 || part == 2)
    {
        for (frame = 0; frame < frameNum; frame++)
        {
            ...
            MarchCubeCUDA<<<numBlocks, numThreads>>>(domain_d, cubeSize_d, twist, 0, meshVertices_d, meshNormals_d, cubesPerThread);
    ...
    if (part == -1 || part == 3)
    {
        ...
        //                     Launch the kernel                     //
        ///////////////////////////////////////////////////////////////
        MarchCubeCUDAMultiframe<<<numBlocks, numThreads>>>(domain_d, cubeSize_d, frameNum, twist, maxTwist, 0, meshVertices_d, meshNormals_d, cubesPerThread);
        checkCudaErrors(cudaDeviceSynchronize());
        end = high_resolution_clock::now();
        kernelTime = (duration<double>(end - start)).count();
    ...
    cudaFree(meshVertices_d);
    cudaFree(meshNormals_d);
    checkCudaErrors(cudaMalloc((void **)&meshVertices_d, frameSize * 2 * sizeof(float3)));
    checkCudaErrors(cudaMalloc((void **)&meshNormals_d, frameSize * 2 * sizeof(float3)));
    cudaStream_t stream1, stream2;
    cudaError_t result1, result2;
    result1 = cudaStreamCreate(&stream1);    result2 = cudaStreamCreate(&stream2);
    if (part == -1 || part == 4)
    {
        int offset = 0;
        start = high_resolution_clock::now();
        for (frame = 0; frame < frameNum; frame++)
        {
            ...
            ///////////////////////////////////////////////////////////
            if (frame % 2 == 0){
                MarchCubeCUDATwoPointers<<<numBlocks, numThreads, 0, stream1>>>(domain_d, cubeSize_d, twist, 0, meshVertices_d, meshNormals_d, cubesPerThread);
            } else {
                MarchCubeCUDATwoPointers<<<numBlocks, numThreads, 0, stream1>>>(domain_d, cubeSize_d, twist, 0, meshVertices_d + frameSize, meshNormals_d + frameSize, cubesPerThread);
            }
            if (frame % 2 == 1){
                ...
                cudaMemcpyAsync(meshNormals_h + offset - frameSize, meshNormals_d, frameSize * sizeof(float3), cudaMemcpyDeviceToHost, stream2);
                cudaMemset(meshNormals_d, 0, frameSize * sizeof(float3));
                cudaMemset(meshVertices_d, 0, frameSize * sizeof(float3));
            } else {
                if (frame > 0){
                    ...
                    cudaMemcpyAsync(meshNormals_h + offset - frameSize, meshNormals_d + frameSize, frameSize * sizeof(float3), cudaMemcpyDeviceToHost, stream2);
            ...
            if (frame == frameNum - 1){
                ...
                if (frame % 2 == 0) lastFrameOffset = frameSize;
                cudaMemcpyAsync(meshVertices_h + offset, meshVertices_d + frameSize - lastFrameOffset, frameSize * sizeof(float3), cudaMemcpyDeviceToHost, stream2);
                cudaMemcpyAsync(meshNormals_h + offset, meshNormals_d + frameSize - lastFrameOffset, frameSize * sizeof(float3), cudaMemcpyDeviceToHost, stream2);
            }
            // save the object file if told so
            if (saveObj)
            {
    ...
    result1 = cudaStreamDestroy(stream1);    result2 = cudaStreamDestroy(stream2);
    ///////////////////////////////////////////////////////////////////
    //                         Free memory                           //
    ///////////////////////////////////////////////////////////////////
    ...
    cudaFree(domain_d);
    cudaFree(cubeSize_d);
    cudaFree(meshVertices_test);
    cudaFree(meshNormals_test);
}